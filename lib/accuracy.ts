...
const solves: Record<
  string,
  {
    'find it': boolean;
    'fix it': boolean;
    attempts: { 'find it': number; 'fix it': number };
  }
> = {};
...
function calculateAccuracy(challengeKey: string, phase: Phase) {
  ...
  logger.info(
    `Accuracy for '${
      phase === 'fix it' ? 'Fix It' : 'Find It'
    }' phase of coding challenge ${colors.cyan(challengeKey)}: ${
      accuracy > 0.5
        ? colors.green(accuracy.toString())
        : accuracy > 0.25
        ? colors.yellow(accuracy.toString())
